...
import { Mail, MessageSquare, Sparkles, BookOpen, History as HistoryIcon, Settings as SettingsIcon, LogOut, User as UserIcon, LogOutIcon } from 'lucide-react';
  ...
  useEffect(() => {
    const verifySession = async () => {
      try {
        ...
        // res may be normalized ({ status, data, msg }) or axios response
        const user = (res && res.data && res.data.email) ? res.data : (res && res.data && res.data.data && res.data.data.email ? res.data.data : null);
        if (user) {
          try {
          ...
          // Also ensure HubSpot association when portalId is available
          if (params && params.portalId) {
            try {
              const hub = await hasHubspot(params.portalId);
              const has = hub && hub.data === true;
              if (!has) {
                navigateWithParams('/hubspot/install');
              }
            } catch {}
          }
          return;
        }
        // If not user and there is no token, redirect
        ...
        // In case axios is configured to reject
        ...
        <div className="text-center mb-6 relative hero-aura">
          ...
          <div className="flex items-center justify-center gap-2 mb-2 glitter-container">
            ...
            <h1
             ...
             onClick={()=>{
              window.open('https://www.nexce.io', '_blank');
             }}
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium transition-all duration-200 first:rounded-l-xl last:rounded-r-xl ${
                      isActive ? 'text-white shadow-md bg-[linear-gradient(135deg,_#5a6de0_0%,_#667eea_60%,_#8ea2ff_100%)]' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'
                    }`
                  }
                >
